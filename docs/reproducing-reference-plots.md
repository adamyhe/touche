...
The full raw outputs from running this workflow end to end are archived on
Zenodo: <https://zenodo.org/records/21285364>.
...
`--method legacy_fisher --decay-model legacy` is required here, and both
flags matter. `touche` defaults to the calibrated binomial test over a
corrected background density; the method sets the p-value column and the
decay model sets the expected-count columns, so reproducing the published
table byte for byte needs the reference setting of each. The violin plot
reads the observed/expected ratio, so it does shift with `--decay-model`;
the pair-type assignment does not.
...
![Violin plot of normalized contacts by pair type](images/local-decay-violinplot.png)
...
| FLV vs DMSO | TRP vs DMSO |
| --- | --- |
| ![FLV vs DMSO observed/expected APA heatmap](images/apa-flv-vs-dmso-obsoverexp.png) | ![TRP vs DMSO observed/expected APA heatmap](images/apa-trp-vs-dmso-obsoverexp.png) |
...
| FLV vs DMSO | TRP vs DMSO | TRP vs FLV |
| --- | --- | --- |
| ![FLV vs DMSO EP/background scatterplot](images/background-flv-vs-dmso.png) | ![TRP vs DMSO EP/background scatterplot](images/background-trp-vs-dmso.png) | ![TRP vs FLV EP/background scatterplot](images/background-trp-vs-flv.png) |
